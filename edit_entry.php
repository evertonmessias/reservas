...
//
// For example if you want to add a column recording ithe number of participants
...
$entry_fields = array(
  'name', 'description', 'start_date', 'end_date', 'areas',
...
function checkGroupCanEditEntry($group, $rooms)
{
  foreach ($rooms as $room) {
    $stringJson = file_get_contents("groupsPerms/canBookGroupsRoom$room.json");
    if (!$stringJson) continue;
    $arrayGroups = json_decode($stringJson, TRUE);
    if (!in_array($group, $arrayGroups) || empty($group))
      showAccessDenied(date("d"), date("m"), date("Y"), "", $room);
    return FALSE;
  }
  return TRUE;
}
...
function create_field_entry_description($disabled = FALSE)
{
  ...
  if (
    isset($select_options['entry.description']) ||
...
function create_field_entry_confirmation_status($disabled = FALSE)
{
  ...
  if ($confirmation_enabled) {
    ...
    $params = array(
      'label'       => get_vocab("confirmation_status") . ":",
      'name'        => 'confirmed',
      'value'       => ($confirmed) ? 1 : 0,
      'options'     => $buttons,
      'force_assoc' => TRUE,
...
function create_field_entry_privacy_status($disabled = FALSE)
{
  ...
  if ($private_enabled) {
    ...
    $params = array(
      'label'       => get_vocab("privacy_status") . ":",
      'name'        => 'private',
      'value'       => ($private) ? 1 : 0,
      'options'     => $buttons,
      'force_assoc' => TRUE,
...
function create_field_entry_custom_field($field, $key, $disabled = FALSE)
{
  ...
  // VERIFICAR VIDEOCONFERENCIA SEMPRE ATIVO ????
...
if ($auth['type'] == "ldap" && !$is_admin) {
  $rodei = checkGroupCanEditEntry($_SESSION['UserGroup'], (isset($room) ? array($room) : $selected_rooms));
}
    ...
    if (($edit_type == "series") && $repeats_allowed) {
      ...
      if (!$disabled || ($rep_type != REP_NONE)) {
        ...
        if (!$disabled) {
          echo "<div>\n";
          $params = array(
            'label' => get_vocab("skip_conflicts") . ":",
            'name' => 'skip',
