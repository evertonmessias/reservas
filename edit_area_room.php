...
$dependence = get_form_var('dependence', 'int');
$canBookGroups  = get_form_var("can_book_room",'array');
  ...
  if (isset($change_room) && !empty($room))
  {
    ...
    if (FALSE != $valid_email)
    {
      ...
      else
      {
        ...
        //Adding logic for creating a new dependence between the current room and another one
	if($dependence  > 0){
	  $sqlDependence = "INSERT INTO mrbs_dependence (dependenceFrom,dependenceTo) VALUES(?,?)";
          $dependenceParams = array($dependence,$room);
          if($isMutual == 1){
             $sqlDependence.=",(?,?)";
             $dependenceParams+= array($room,$dependence);
          }

	  db()->command($sqlDependence, $dependenceParams);
	}
	if(!empty($canBookGroups)){
	   $stringJson = json_encode($canBookGroups);
	   $handle = fopen("groupsPerms/canBookGroupsRoom$room.json","w")or die("canBookGroupsRoom$room.json");
	   fwrite($handle,$stringJson);
	   fclose($handle);
	}
...
if (isset($change_room) && !empty($room))
{
  ...
  <form class="form_general" id="edit_room" action="edit_area_room.php" method="post">
    ...
    <legend></legend>
      ...
      echo "</div>\n";
      if($is_admin){
       $rooms = array(-1=>'Selecione...');
       $rooms = $rooms + get_room_names($room_data['area_id']);
       $foundDependences = getDependences($room_data['id']);
 
	//var_dump($foundDependences);var_dump($rooms);
       unset($rooms[$room_data['id']]);
       foreach($foundDependences as $row){
       	$index = array_key_exists($row['sala'],$rooms);
	if(in_array($row['sala'],array_keys($rooms)))
       	  unset($rooms[$row['sala']]);
       }
      echo "<div>\n";
	$foundDependences = getDependences($room_data['id'],$onlyDependenceTo=FALSE);
	//var_dump($foundDependences);
	foreach($foundDependences as $depRoom){
		echo "<p>".get_room_name($depRoom['sala'])."
		     <a href=\"del.php?type=dependence&dep=$depRoom[id]\">	
		      <img src=\"images/delete.png\" width=\"16\" height=\"16\" 
                         alt=\"" . get_vocab("delete") ." ". strtolower(get_vocab('dependence'))."\"
                         title=\"" . get_vocab("delete")  ." ". strtolower(get_vocab('dependence'))."\"></a>\n</p>";
	}
      echo "</div>\n";
      echo "<div>\n";
       $params = array('label'         => get_vocab("dependence") . ":",
                       'name'          => 'dependence',
                       'options'       => $rooms,
                       'force_assoc'   => TRUE,
                       'value'         => '',
	...
	//var_dump($_SESSION);
       echo "</div>\n";
       if(!empty($ldap_perm_groups)){
         echo "<div>";
	 $stringJson = file_get_contents("groupsPerms/canBookGroupsRoom$room_data[id].json");
	 $arrayGroups = array();
	 if($stringJson)
            $arrayGroups = json_decode($stringJson,TRUE);
         $params = array('label'         => "Podem reservar esta sala:",
                        'label_title'   => get_vocab("disabled_room_note"),
                        'name'          => "can_book_room[]",
                        'value'         => (!empty($arrayGroups)?$arrayGroups:""),
                        'options'       => $ldap_perm_groups,
                        'force_assoc'   => TRUE,
                        'disabled'      => $disabled,
                        'create_hidden' => FALSE);
